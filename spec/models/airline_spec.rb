...
RSpec.describe Airline do
  ...
  describe 'class methods' do
    before :each do
      ...
      @flight3 = @airline1.flights.create!(number: 3, date: "10/03/20", departure_city: "Asheville", arrival_city: "Miami")
      @passenger1 = Passenger.create!(name: "Best Passenger", age: 25)
      @passenger2 = Passenger.create!(name: "Annoying Passenger", age: 55)
    ...
    it 'does not show duplicate passengers' do
      expect(@airline1.all_passengers.count).to eq(3)
    end

    it 'does not show passengers under age 18' do
      expect(@airline1.all_passengers).to eq([@passenger1, @passenger2, @passenger3])
    end

    it 'orders passengers by number of flights' do
      expect(@airline1.all_passengers).to eq([@passenger1, @passenger2, @passenger3])
    end
  end
end
